...
async def lifespan(app: FastAPI):
    ...
    It ensures that GPU memory is cleared after the app's lifecycle ends, which is 
    essential for efficient resource management in GPU environments.
    """
    yield
...
async def list_models():
    ...
    This is useful for clients to query and understand what models are available for 
    use.
    """
    model_card = ModelCard(id="cogagent-chat-hf")
...
async def predict(model_id: str, params: dict):
    """
    Handle streaming predictions. It continuously generates responses for a given input 
    stream. This is particularly useful for real-time, continuous interactions with the 
    model.
    """
...
def generate_cogvlm(
    ...
    """
    Generates a response using the CogVLM model. It processes the chat history and image
    data, if any, and then invokes the model to generate a response.
    """
...
def generate_stream_cogvlm(
    ...
    It's optimized to handle continuous input-output interactions with the model in a 
    streaming manner.
    """
...
if __name__ == "__main__":
    ...
    if "cuda" in DEVICE:
        # Update code according to available GPUs
        # Reference: https://huggingface.co/THUDM/cogvlm-chat-hf
        ...
        device_map = infer_auto_device_map(
            model,
            max_memory={0: "21GiB", 1: "21GiB", "cpu": "16GiB"},
            no_split_module_classes=["CogAgentDecoderLayer", "TransformerLayer"],
        )
